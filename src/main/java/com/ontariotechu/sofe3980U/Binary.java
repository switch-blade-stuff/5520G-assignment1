...
public class Binary {
	...
	public static Binary mul(Binary num1, Binary num2) {
		...
		int off = 0;
		while (ind2 >= 0) {
			String digits = "0".repeat(off);
			int i2 = (num2.number.charAt(ind2) == '1') ? 1 : 0;
			int i3 = 0;
			int ind1 = num1.number.length() - 1;
			while (ind1 >= 0) {
				i3 = i2 * ((num1.number.charAt(ind1) == '1') ? 1 : 0);
				ind1--;
				digits = (i3 == 0 ? "0" : "1") + digits;
			}
			nums.push(digits);
			ind2--;
			off++;
		}
		Binary num3 = new Binary(nums.pop());
		while (!nums.empty()) {
			num3 = add(num3, new Binary(nums.pop()));
		}
		return num3;
	}
	...
	public static Binary or(Binary num1, Binary num2) {
		...
		while (ind1 >= 0 || ind2 >= 0) {
			boolean bit = false;
			if (ind1 >= 0) {
				bit |= num1.number.charAt(ind1) == '1';
				ind1--;
			}
			if (ind2 >= 0) {
				bit |= num2.number.charAt(ind2) == '1';
				ind2--;
			}
			num3 = (bit ? "1" : "0") + num3;
		}
